...
public final class CentredMessage {
    ...
    private final static String barTranslated = ChatColor.translateAlternateColorCodes('&', MessageManager.getMessage("reset.kick.bars"));
    private final static String bar = ChatColor.stripColor(barTranslated);
    private final static int CENTER_PX = measureSize(bar) / 2;

    public static int measureSize(String message) {
        int messagePxSize = 0;
    ...
    public static String generate(String message) {
        ...
        int messagePxSize = measureSize(message);
        ...
        String toAppend = StringFormatter.repeat(ChatColor.BLACK + ".", Math.abs(diff));
        return out + toAppend;
    }
